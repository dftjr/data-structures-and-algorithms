...
const standardizePhoneNumbers = (arr) => {
  return arr.map(num => {
    ...
    let middleNumbers = num.substring(6, 9);
    let endNumbers = num.substring(10);
    return areaCode + middleNumbers + endNumbers;
  });
};
...
const onlyOddChars = (str) => {
  ...
  return str.split('').filter((character, idx) => idx % 2 === 1).join('');
};
