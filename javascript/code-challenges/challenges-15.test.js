...
const toTitleCase = (arr) => {
  return arr.map(str => {
    let firstLetterArr = str.charAt(0).toUpperCase();
    return firstLetterArr + str.substr(1);
  })
};
...
const detectTicTacToeWin = (board) => {
  let checkBoardSquare = (set) => {

    // Check if values are blank && if there is something in the cell, see if it matches
    return set.every(cell => cell !== '' && cell === set[0])
      ? true
      : false;
  };
  // Row checks: top, middle, bottom
  if (checkBoardSquare(board[0])) return true;
  if (checkBoardSquare(board[1])) return true;
  if (checkBoardSquare(board[2])) return true;

  // Column checks: left, middle, right
  if (checkBoardSquare([board[0][0], board[1][0], board[2][0]])) return true;
  if (checkBoardSquare([board[0][1], board[1][1], board[2][1]])) return true;
  if (checkBoardSquare([board[0][2], board[1][2], board[2][2]])) return true;

  // Diagonal checks: right side, left side
  if (checkBoardSquare([board[0][0], board[1][1], board[2][2]])) return true;
  if (checkBoardSquare([board[0][2], board[1][1], board[2][0]])) return true;
